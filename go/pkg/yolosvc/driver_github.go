...
func getOverrideBuildpb(owner string, repo string, artiID int64, token string) (*yolopb.Build, error) {
	override := &yolopb.Build{}
...
func (worker *githubWorker) batchFromWorkflowRun(run *github.WorkflowRun, prs []*github.PullRequest, overrideRun *yolopb.Build) *yolopb.Batch {
	batch := yolopb.NewBatch()
	...
	proto.Merge(&newBuild, overrideRun)
